<nav class="nav_lateral large-3 medium-4 columns" id="actions-sidebar">
    <ul class="side-nav">
        ...
        <li><?= $this->Form->postLink(
                __('Deletar'),
                ['action' => 'delete', $talao->id],
                ['confirm' => __('Tem certeza que deseja deletar #{0}?', $talao->id)]
            )
        ?></li>
...
<div class="bg_forms taloes form large-9 medium-8 columns content">
    ...
    <fieldset>
        <legend><?= __('Editar Talão') ?></legend>
        
             <div class='bg_img_form'>
            
            <?= $this->Html->image("produtos/".$talao->img_produto, ['id'=>'img_form']);?>
